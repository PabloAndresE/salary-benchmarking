...
| Un **corte duro de similitud** para entrar al vecindario | Hoy no existe: los 25 mas cercanos entran siempre, con poco peso si estan lejos. `LOTERO` se contesta con `TESORERO`, `BOTERO` y `LAMPERO`, y el unico aviso es que el intervalo se ensancha. .Mejora descartarlos en vez de despesarlos? Mismo diseno que D-021: pinball, pareado y placebo | 1 experimento |
...
| El sesgo de una pasada en **`efecto_nivel`** | Centra por empresa, luego por area, y toma la media por nivel. Con areas desbalanceadas en composicion de niveles eso deja sesgo; lo correcto es un ajuste de dos vias. **Sigue vivo tras D-024**, que cambio el ESTIMADOR y no el esquema de centrado | 1 experimento |
| **Repetir D-025 con potencia** | La fusion por errata subio la cobertura de los titulos afectados del 0% al 100%, pero el placebo salio no concluyente con n=64 votos. Estimaciones puntuales en la direccion predicha (real -0,0148, placebo +0,0177) y ningun IC excluye el cero. Hacen falta varias particiones y el contraste directo real-contra-placebo | 5 particiones x 3 bases, ~7 h |
| La decision sobre **genero** en la fusion | 961 pares sueltos (`VENDEDOR`/`VENDEDORA`) mas los que la semantica YA junta sola (`ENFERMERAS`/`ENFERMEROS` ya comparten grupo desde D-015). El argumento es de equidad, no estadistico: celdas separadas hacen que a una `ENFERMERA` se la compare contra un centro 21% menor que el de `ENFERMERO` | 1 experimento + D-026 |
